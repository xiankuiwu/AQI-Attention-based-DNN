...
class NeuralBeats:   
    def __init__(
            self,forecast_length,data=None,
            backcast_length=None,
            save_model=False,
            path='',
            checkpoint_name='NBEATS-checkpoint.pth',
            mode='cpu',
            batch_size=None,
            thetas_dims=[4, 8],
    ...
    def eval_test(self, test_losses, x_test, y_test):
        self.net.eval()
        _, forecast = self.net(torch.tensor(x_test, dtype=torch.float,device=self.device))
        test_losses.append(F.mse_loss(forecast, torch.tensor(y_test, dtype=torch.float,device=self.device)).item())
